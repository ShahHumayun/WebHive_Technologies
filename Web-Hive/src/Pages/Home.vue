...
const technologies = ['Magento', 'React Native', 'PHP', 'Laravel', 'Next.js']
...
.hero-wrapper {
  ...
  inset: 0;
  height: 100vh;
  /* FIX: Use dvh where supported so mobile browser chrome doesn't clip the layout */
  height: 100dvh;
  max-height: 100dvh;
  width: 100vw;
  overflow: hidden !important;
...
.nav-overlay {
  ...
  width: 100%;
  height: 100%;
  /* FIX: Use dvh for proper mobile fullscreen */
  height: 100dvh;
  overflow: hidden !important;
  /* FIX: Safe padding for notch/home bar on iOS */
...
.nav-links-container {
  display: flex;
  flex-direction: column;
  /* FIX: Use clamp with vh so items don't overflow on short screens */
...
.menu-index {
  ...
  /* FIX: Slightly larger floor for readability on small screens */
  font-size: clamp(12px, 1.5vw, 14px);
  color: var(--brand-accent);
  /* FIX: Fixed min width so numbers don't jump */
...
.menu-link {
  /* FIX: Tighter floor — prevents overflow on 320px wide phones */
  ...
  display: inline-block;
  line-height: 1.1;
  /* FIX: Ensure long words break if needed */
...
.consult-btn-overlay {
  ...
  border-radius: 5px;
  font-weight: 800;
  /* FIX: Use vw-based clamp instead of vh — prevents tiny size in landscape */
...
/* FIX: Desktop — left-align overlay menu */
...
/* FIX: Landscape mobile — reduce font size further so items fit without scroll */
...
.hero-main {
  ...
  width: 100%;
  max-width: 100%;
  /* Do NOT clip — clipping was hiding the subtitle */
  overflow: visible;
}
...
.main-title {
  /* Controlled size: never overflows its container on any screen */
  font-size: clamp(3rem, 10vw, 8rem);
  font-weight: 950;
  letter-spacing: -0.04em;
  /* Slightly more line-height so descenders don't cut into subtitle */
  line-height: 1;
  text-transform: uppercase;
  pointer-events: auto;
  ...
  text-align: center;
  word-break: break-word;
  /* Prevent title block from ever growing outside hero-main */
  flex-shrink: 1;
}
...
.hero-subtitle {
  ...
  /* margin-top removed — parent gap handles spacing */
  width: 100%;
  max-width: 520px;
  ...
  /* Subtitle should never shrink away */
  flex-shrink: 0;
}
...
.marquee-footer {
  ...
  /* INCREASED HEIGHT: Padding changed from 12px to 24px */
  padding: 24px 0;
  overflow: hidden;
  position: relative;
...
.marquee-item {
  ...
  letter-spacing: 0.2em;
  text-transform: uppercase;
  /* FIX: Better floor for readability on small screens */
